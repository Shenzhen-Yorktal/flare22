...
class PlainConvUNetEncoder(nn.Module):
    def __init__(self, input_channels, base_num_features, num_blocks_per_stage, feat_map_mul_on_downscale,
        ...
        assert len(pool_op_kernel_sizes) == len(conv_kernel_sizes) - 1
        ...
        for stage in range(num_stages):
            ...
            if stage == 0:
                current_pool_kernel_size = [1, 1, 1]
            else:
...
class PlainConvUNetMultiDecoder(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_blocks_per_stage_encoder, feat_map_mul_on_downscale,
                 ...
                 deep_supervision=False, upscale_logits=False, max_features=512, initializer=None, num_decoders=1, training=True):
        super(PlainConvUNetMultiDecoder, self).__init__()
        ...
        for i in range(num_decoders):
            decoders.append(PlainConvUNetDecoder(self.encoder, num_classes, num_blocks_per_stage_decoder, props,
    ...
    def forward(self, x):
        if self.training:
            ...
            for idx in range(self.num_decoders):
                out1 = self.decoders[idx](skips)
                out.append(out1)
                temp_seg = [temp_prob.detach().argmax(1) for temp_prob in out1]
                predict_seg.append(temp_seg)

            return out, predict_seg
        else:
            skips = self.encoder(x)
            return self.decoders[0](skips)[0]
    ...
    def compute_reference_for_vram_consumption_3d():
        ...
        return PlainConvUNetMultiDecoder.compute_approx_vram_consumption(patch_size, 32, 512, 4, 3, pool_op_kernel_sizes,
...
if __name__ == "__main__":
    ...
    patch_size = (256, 256)
    batch_size = 56
    unet = PlainConvUNetMultiDecoder(4, 32, (2, 2, 2, 2, 2, 2, 2), 2, pool_op_kernel_sizes, conv_op_kernel_sizes,
